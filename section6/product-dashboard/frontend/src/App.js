...
function HomeView() {
	return (
		<section style={styles.card}>
			...
			<p>
				This demo dashboard shows a React frontend integrated with a
				Node.js/Express API.
			</p>
			<ul style={styles.list}>
				<li>Browse and add products in the Products view.</li>
				<li>
					Check aggregated metrics and backend instance in the Stats view.
				</li>
			</ul>
		</section>
	);
...
function StatsView() {
	...
	const loadStats = React.useCallback(async () => {
		try {
			...
			const data = await response.json();
			setStats(data);
			setLastCacheStatus(response.headers.get("X-Cache") || "-");
		} catch (err) {
